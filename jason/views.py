...
from jason.models import 인편_jason
from django.views.generic.edit import CreateView
from django.utils import timezone
from jason import the_campy_sender_jason
...
def index(request) :
    인편_latest = 인편_jason.objects.order_by('-작성일')[:5]
    context = {
    "인편_latest" : 인편_latest
    }
    return render(request,"index(jason).html", context = context)

def writer(request):
    if request.method == 'GET':
        return render(request, 'letterwrite(jason).html')
    ...
    if request.method == 'POST':
        ...
        보낼값 = 인편_jason(  # 모델에서 생성한 클래스를 가져와 객체를 생성 (모델항목과 html 에서 받아온 값을 연결해주는 역할)
            작성자=작성자,
            제목=제목,
            내용=내용,
        )
        ...
        the_campy_sender_jason.send(작성자, 제목, 내용)
